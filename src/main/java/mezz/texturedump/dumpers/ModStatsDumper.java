...
public class ModStatsDumper {
    ...
    public ModStatsDumper() {
        for (Map.Entry<String, ModContainer> modEntry : Loader.instance()
            .getIndexedModList()
            .entrySet()) {
    ...
    public Path saveModStats(String name, TextureMap map, Path modStatsDir) throws IOException {
        ...
        Map<String, Long> modPixelCounts = (Map<String, Long>) map.mapUploadedSprites.values()
            .stream()
            .collect(
                Collectors.groupingBy(
                    sprite -> new ResourceLocation(((TextureAtlasSprite) sprite).getIconName()).getResourceDomain(),
                    Collectors.summingLong(
                        sprite -> (long) ((TextureAtlasSprite) sprite).getIconWidth()
                            * ((TextureAtlasSprite) sprite).getIconHeight())));

        final long totalPixels = modPixelCounts.values()
            .stream()
        ...
        List<Map.Entry<String, Long>> sortedEntries = modPixelCounts.entrySet()
            .stream()
            .sorted(Collections.reverseOrder(Map.Entry.comparingByValue()))
            .collect(Collectors.toList());

        ProgressManager.ProgressBar progressBar = ProgressManager
            .push("Dumping Mod TextureMap Statistics", sortedEntries.size());
